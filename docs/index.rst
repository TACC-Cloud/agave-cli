.. Tapis-CLI documentation master file, created by
   sphinx-quickstart on Mon Oct 29 14:00:12 2018.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to the Tapis-CLI documentation!
=======================================

Tapis-CLI is your command-line friendly utility to interact with TACC's Agave
plaform. Tapis-CLI is powered by `AgavePy <https://github.com/TACC/agavepy>`_.

.. toctree::
    :maxdepth: 2
    :caption: Installation, Upgrade and Configuration

    docsite/installation_guide/index


.. toctree::
    :maxdepth: 2
    :caption: Contributing to AgavePy

    docsite/contributing_guide/index


.. toctree::
   :maxdepth: 1
   :caption: Using AgavePy

   docsite/authentication/index
   docsite/files/index

.. toctree::
    :maxdepth: 1
    :caption: Advanced Usage

    docsite/client_management/index


Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
