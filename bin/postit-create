...
interactive_opts=(apisecret apikey url method)
...
Description of this script.
 ...
 Options:
  ...
  -U, --url         Target URL to which the postit points
  -M, --method      HTTP method the postit should perform: GET, POST, PUT, DELETE
  -N, --noauth      Should the postit be preauthenticated?
...
main() {
	...
	if echo "$method" | grep -E "[GET|get|PUT|put|POST|post|DELETE|delete]" >/dev/null ; then
		post_options="method=$method&$post_options"
	else
...
format_api_json() {
	...
	if ((verbose)); then
		...
		jsonval postit_url "$1" "postit" 
		success "https:$postit_url"
	fi
}
