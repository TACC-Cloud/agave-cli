...
main() {
	...
	if [ -z "$args" ]; then
		...
		cmd="curl -sku \"$apisecret:XXXXXX\" -X POST -F \"fileToUpload=@$filetoupload\" $hosturl$args/credentials/$internalUsername"

		log "Calling $cmd"

		response=`curl -sku "$apisecret:$apikey" -X POST -F "fileToUpload=@$filetoupload" "$hosturl$args/credentials/$internalUsername"`
...
format_api_json() {
	...
	if ((verbose)); then
		...
		jsonval internalusername "$i" "internalUsername"
