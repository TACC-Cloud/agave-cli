...
Description of this script.
 ...
 Options:
  ...
  -u, --internalusername Internal user to delete
...
main() {
	...
	cmd="curl -sku \"$apisecret:XXXXXX\" -X DELETE $hosturl$apisecret/users/$internalUsername"
	
	log "Calling $cmd"
		
	response=`curl -sku "$apisecret:$apikey" -X DELETE $hosturl$apisecret/users/$internalUsername`
...
format_api_json() {
	...
	if ((verbose)); then
		...
		if [ -n "$internalUsername" ]; then
			success "Successfully deleted internal user $internalUsername"
		else
			success "Successfully deleted all internal users"
		fi
	fi
}
