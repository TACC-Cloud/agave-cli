...
main() {
	...
	cmd="curl -sku \"$apisecret:XXXXXX\" -X POST -F \"fileToUpload=@$filetoupload\" $hosturl$apisecret/users/$args"

	log "Calling $cmd"
set -x
	response=`curl -sku "$apisecret:$apikey" -X POST -F "fileToUpload=@$filetoupload" "$hosturl$apisecret/users/$args"`
