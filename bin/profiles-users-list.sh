...
main() {
	...
	cmd="curl -sku \"$apisecret:XXXXXX\" $hosturl$apisecret/users/$internalUsername"

	log "Calling $cmd"
		
	response=`curl -sku "$apisecret:$apikey" $hosturl$apisecret/users/$internalUsername`
